...
public abstract class AbstractZestExample extends Application {
	...
	protected static Edge e(Graph graph, org.eclipse.gef4.graph.Node n, org.eclipse.gef4.graph.Node m, Object... attr) {
	...
	protected static Edge e(org.eclipse.gef4.graph.Node n, org.eclipse.gef4.graph.Node m, Object... attr) {
		String label = (String) n.getAttrs().get(LABEL) + (String) m.getAttrs().get(LABEL);
		Edge.Builder builder = new Edge.Builder(n, m).attr(LABEL, label).attr(ID, genId());
	...
	@Override
	public void start(final javafx.stage.Stage primaryStage) throws Exception {
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				graph = createGraph();
				viewer.getAdapter(ContentModel.class).setContents(Collections.singletonList(graph));
			}
		});
